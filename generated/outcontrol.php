...
use margusk\Warbsorber\Functions\Exceptions\OutcontrolException;
...
{
    Call::invoke('ob_clean',OutcontrolException::class,false);
}
...
{
    Call::invoke('ob_end_clean',OutcontrolException::class,false);
}
...
{
    Call::invoke('ob_end_flush',OutcontrolException::class,false);
}
...
{
    Call::invoke('ob_flush',OutcontrolException::class,false);
}
...
function ob_start($callback = null, int $chunk_size = 0, int $flags = PHP_OUTPUT_HANDLER_STDFLAGS): void
{
    if ($flags !== PHP_OUTPUT_HANDLER_STDFLAGS) {
        Call::invoke('ob_start',OutcontrolException::class,false, $callback, $chunk_size, $flags);
    } elseif ($chunk_size !== 0) {
        Call::invoke('ob_start',OutcontrolException::class,false, $callback, $chunk_size);
    } elseif ($callback !== null) {
        Call::invoke('ob_start',OutcontrolException::class,false, $callback);
    } else {
        Call::invoke('ob_start',OutcontrolException::class,false);
    }
}
...
{
    Call::invoke('output_add_rewrite_var',OutcontrolException::class,false, $name, $value);
}
...
{
    Call::invoke('output_reset_rewrite_vars',OutcontrolException::class,false);
}
